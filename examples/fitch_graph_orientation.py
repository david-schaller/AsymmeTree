...
def simulation(arg):
    ...
    S = te.species_tree_n_age(
        np.random.randint(S_min, S_max + 1), 1.0, model="yule", innovation=True
    )
    ...
    for T in (OGT, cotree, lrt, brt):
        ...
        for matrix in [
            hgt.fitch_orientation(T, P, lca=lca_T),
...
if __name__ == "__main__":
    ...
    tasks = [(i, *item) for i, item in enumerate(itertools.product(range(repeats), rates))]
