...
\chapter{The Gene Family Simulator}
...
\end{equation}
The distribution of the waiting time after the speciation event is thus equal to an exponential distribution with rate parameter $R_2$. It is the same that will be used in the following iteration. The respective probability density functions are visualized in Figure~\ref{fig:exponential_dist}.
\vspace{0.5mm}
\begin{figure}[ht]
	\begin{center}     
		\includegraphics[width=\columnwidth]{exponential_dist.pdf}
	\end{center}
	\caption[Visualization of the probability density functions for the waiting times $\Delta\tau$ and~$\Delta\tau'$ until the next event]{Visualization of the probability density functions (or complementary cumulative distribution function, respectively)\ for the waiting times~$\Delta\tau$ and~$\Delta\tau'$ until the next event. A speciation occurs after a time interval of length $\sigma$. Under the condition that no event occurred until the speciation (i.e., $\Delta\tau>\sigma$), a new waiting time $\Delta\tau'$ can be drawn with the time point $\tau_s$ of the speciation as offset.}
