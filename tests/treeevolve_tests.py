...
class TestTreeEvolvePackage(unittest.TestCase):
    def test_species_tree(self):
        ...
        for model in ("yule", "BDP", "EBDP"):
            ...
            leaves = [leaf for leaf in species_tree.leaves() if leaf.event != "L"]
    ...
    def test_no_extinction(self):
        ...
        for _ in range(repeats):
            species_tree = te.species_tree_n_age(n, 1.0, model="yule", contraction_probability=0.2)
            ...
            te.rate_heterogeneity(gene_tree, species_tree, autocorr_variance=0.2, inplace=True)

            # check that there is no extinction in any species
            reconc_dict = {
                leaf.label: []
                for leaf in species_tree.preorder()
                if not leaf.children and leaf.event != "L"
            }
            ...
            self.assertTrue([leaf for leaf in gene_tree2.leaves()])
