...
PATH_CUSTOM_MATRIX = Path(__file__).parent / "custom.paml"
...
class TestSeqEvolvePackage(unittest.TestCase):
    def test_custom_matrix(self):
        ...
        subst_model = se.SubstModel("a", "CUSTOM", filename=PATH_CUSTOM_MATRIX)
    ...
    def test_simulation(self):
        ...
        evolver = se.Evolver(
            subst_model, indel_model=indel_model, het_model=het_model, gillespie=False
        )
    ...
    def test_simulation_gillespie(self):
        ...
        evolver = se.Evolver(
            subst_model, indel_model=indel_model, het_model=het_model, gillespie=True
        )
        evolver.evolve_along_tree(species_tree, start_length=150)
