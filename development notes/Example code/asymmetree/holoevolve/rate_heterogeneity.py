...
def gene_trees(
    ...
    **kwargs: object,
    ...
    for _ in range(n):
        ...
        rate_heterogeneity(
            TGT, S, base_rate=base_rate_sampler(), autocorr_factors=autocorr_factors, **kwargs
        )
...
def rate_heterogeneity(
    ...
    **kwargs,
    ...
    _divergent_rates(
        T, S, Sampler(rate_increase, shift=1.0), np.asarray(CSN_weights) / sum(CSN_weights)
    )
    ...
    if autocorr_factors:
        for v in T.preorder():
            if v.parent:
                edge_ID = v.reconc[1] if isinstance(v.reconc, (tuple, list)) else v.reconc
...
def _divergent_rates(T: Tree, S: Tree, sampler: Sampler, CSN_weights: tuple) -> Tree:
    ...
    # nodes will be marked as conserved or divergent
    ...
    for u in T_nodes:
        ...
        if u.event in ("S", "", None):
            for v in u.children:
                marked[v] = marked[u]
                S_u = u.reconc
                S_v = v.reconc if not isinstance(v.reconc, (tuple, list)) else v.reconc[1]
        ...
        # ---------- HORIZONTAL GENE TRANSFER ----------
            ...
            S_edge = (
                v2.reconc
                if isinstance(v2.reconc, (tuple, list))
