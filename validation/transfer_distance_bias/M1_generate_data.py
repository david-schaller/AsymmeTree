...
def simulation(sim_ID, repeat, rate_combination, bias_mode):
    ...
    S = te.species_tree_n_age(
        np.random.randint(S_min, S_max + 1),
        1.0,
        model="yule",
    ...
    while True:
        ...
        with multiprocessing.Pool(processes=1) as pool:
            ...
            try:
                ...
                with file_lock:
                    write_line(outfile, outfile2, line, distances_repl, distances_add)
...
if __name__ == "__main__":
    ...
    tasks = [
        (i, *item)
        for i, item in enumerate(itertools.product([i for i in range(repeats)], rates, bias_modes))
    ]
    ...
    threads = [
        threading.Thread(target=mp_with_timeout, args=(tasks, file_lock, task_lock, outfile))
