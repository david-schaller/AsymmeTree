        ...
        for d in distances:
            ...
            for subst_model in subst_models:
                ...
                for length in lengths:
                    outfile = "{}/{}_{}_{}_{}.phylip".format(
                        directory, subst_model.model_name, d, length, i
                    )
                    print("Processing '{}'".format(outfile))

                    start_time = time.time()
                    evolver.evolve_along_tree(T, start_length=length)
                    ...
                    if subst_model.model_name in ("WAG", "JTT"):
                        ...
                        write_alignment(outfile, alignment, alignment_format="phylip")

                    else:
                        start_time = time.time()
                        evolver.true_alignment(include_inner=True, write_to=outfile)
                        align_time = time.time() - start_time

                    stats.append((subst_model.model_name, d, length, i, evolve_time, align_time))
...
def calculate_distances(outfile):
    ...
    for file in glob.glob(directory + "/*.phylip"):
        model, d, length, i = os.path.basename(file).rsplit(".", 1)[0].split("_")
        d, length, i = float(d), int(length), int(i)
        print(model, d, length, i)
        ...
        table.append([model, d, length, i, d_hat])
