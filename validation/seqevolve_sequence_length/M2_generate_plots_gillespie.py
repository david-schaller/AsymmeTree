...
df = pd.read_csv("results/all_distances_gillespie.csv")
...
for i in range(len(models)):
    ax_twin = axs[i].twinx()
    ax_twin.axhline(y=1, xmin=0, xmax=1, color="grey", linewidth=0.7)
    ...
    sns.boxplot(y="d_hat", x="d", data=df1, hue="length", ax=axs[i], fliersize=2, linewidth=0.7)

    axs[i].set_ylim(0, 7)
    axs[i].set_ylabel("estimated distance" if i in (0, 1) else "", fontsize=fs)
    ...
    for j in range(len(distances)):
        axs[i].axhline(y=distances[j], xmin=(j) * xw, xmax=(j + 1) * xw, color="r", linewidth=0.7)
    ...
    sns.barplot(y="proportion_nan", x="d", data=df_na, hue="length", ax=ax_twin, palette="pastel")

    ax_twin.legend().set_visible(False)
    ax_twin.set_ylim(0, 7)
    ax_twin.set_ylabel("Proportion NaN " if i in (2, 3) else "", fontsize=fs, loc="bottom")
...
plt.tight_layout()
plt.savefig("results/distance_gillespie_boxplot.pdf", dpi=450)
