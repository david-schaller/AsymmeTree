...
for i in range(repeats):
    ...
    for n in tree_sizes:
        S = te.species_tree_n_age(n, 1.0, model="yule", innovation=True)
        ...
        for length in lengths:
            for j in range(1, 3):
                outfile = os.path.join(seq_directory, f"indelible{j}_{i}_{n}_{length}")

                # configuration files for INDELible
                with open(
                    os.path.join(tree_directory, f"indelible{j}_{i}_{n}_{length}.txt"), "w"
                ) as f:
                    f.write(f"[TYPE] AMINOACID {j}\n[MODEL] m1\n  [submodel] WAG\n")
                    if j == 2:
                        f.write("  [rates] 0 1 5\n")
                    f.write(f"[TREE] t1 {newick2}\n")
                    f.write(f"[PARTITIONS] p1 [t1 m1 {length}]\n")
                    f.write(f"[EVOLVE] p1 1 {outfile}\n")

                wdir = os.path.join(seq_directory, f"alf{j}_{i}_{n}_{length}")
                tree_file = os.path.join(tree_directory, f"tree_{i}_{n}_nolabel")
                rate_var_model = "Gamma, 5, 0, 1" if j == 2 else ""

                # configuration files for INDELible
                with open(os.path.join(tree_directory, f"alf{j}_{i}_{n}_{length}.drw"), "w") as f:
                    f.write(
                        f"# name of simulation\n"
                        "mname := uuid;\n\n"
