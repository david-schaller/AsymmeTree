...
for i, N, length in itertools.product(repeats, tree_sizes, lengths):
    print(i, N, length)
    ...
    for j in range(1, 3):
        outfile = os.path.join(seq_directory, f"{tool}{j}_{i}_{N}_{length}")
        ...
        evolver.evolve_along_tree(tree, start_length=length)
        evolver.write_sequences(outfile, include_inner=False)
        end_time = time() - start_time

        with open(resultfile, "a") as f:
            f.write(f"\n{i},{N},{length},{tool},{j},{end_time}")
    ...
    for j in range(1, 3):
        ...
        if j == 2:
            aa_model = pyvolve.Model("WAG", alpha=het_alpha, num_categories=het_classes)
        else:
            aa_model = pyvolve.Model("WAG")
        partition = pyvolve.Partition(models=aa_model, size=length)
    ...
    for j in range(1, 3):
        ...
        call = f"{binary_path} -m WAG -l {length} {rate_het} < {tree_file}_nolabel > {outfile}"
    ...
    for j in range(1, 3):
        config_file = os.path.join(tree_directory, f"indelible{j}_{i}_{N}_{length}.txt")
    ...
    for j in range(1, 3):
        config_file = os.path.join(tree_directory, f"alf{j}_{i}_{N}_{length}.drw")
