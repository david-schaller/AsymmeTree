# code copied from TreeSim and adjusted such that it also outputs the time
...
sim2.bd.reverse.single <- function(n,lambda,mu,frac){
  ...
  phy2<-reorder(phy2)
  phy2<-list(phy2,time)
  phy2$time <- time       # <---- ADDED THIS LINE
  phy2
}

rates <- c(0.0, 0.2, 0.5, 0.8)

for (r in rates) {
  print(r)
  # trees <- sim.bd.taxa(20, 10, 1.0, r)
  
  ages <- c()
  
  for (i in 1:10000) {
    t <- sim2.bd.reverse.single(20,1.0,r,1.0)
    ages <- c(ages, t$time)
  }
  
  fileConn<-file(paste("results/TreeSim_age", r, ".txt", sep=""))
  writeLines(as.character(ages), fileConn)
  close(fileConn)
}
